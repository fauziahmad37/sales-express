        ...
        <div class="right_col" role="main">
          <div class="row">
            ...
            <div class="x_panel tile">
                <div class="x_title">
                  <h2>Laporan In Out Gudang <?php if($id_workshop !== '') echo '( '.get_array($arrWorkshop, $id_workshop).' - '.date('M Y', strtotime($date)).')';?></h2>
                  <div class="clearfix"></div>
                </div>
                <div class="x_content">
                  <div class="col-md-12 col-sm-12 col-xs-12">
                  <form id="demo-form" autocomplete="off" data-parsley-validate class="form-horizontal form-label-left" action="<?php echo site_url('/Inventory/listInOutInventory/'); ?>" method="post">
				  ...
				  <div class="col-md-12 col-sm-12 col-xs-12 table-responsive" id="content_absen">
						...
						<thead>
							<tr>
								<th class="text-center">Id Inventory</th>
								<th class="text-center">Internal Code Lama</th>
								<th class="text-center">Internal Code Baru</th>
						...
						<?php
							foreach ((Array) $data AS $key => $val) { 
								//$is_sparepart = $val['kode_material'] === null;
									$hargaJual = $val['sale_price'];
									$ppn = $val['ppn_jual_beli'];
									$jual = ($hargaJual * 10 / 100) + $hargaJual;

									echo '<tr>
										<td>'.$val['id_inventory'].'</td>
										<td>'.$val['internal_code'].'</td>
										<td>'.$val['internal_code_baru'].'</td>
										<td>'.$val['kode_part'].'</td>
										<td>'.$val['item_name'].'</td>
										<td>'.$val['in'].'</td>
										<td>'.($val['out'] * -1).'</td>
										...
										<td>'.($val['ppn_jual_beli']).'%</td>
										<td>'.number_format($jual).'</td>
										<td>'.$val['loc_part'].'</td>
									</tr>';
