        ...
        <div class="right_col" role="main">
          <div class="row">
            ...
            <div class="x_panel tile">
                <div class="x_title">
                  <h2>Pembayaran Invoice <?php if($id_workshop !== '') echo '( '.get_array($arrWorkshop, $id_workshop).' - '.date('M Y', strtotime($date)).')';?></h2>
					...
					<table id="datatable" class="display cell-border" style="width:100%">
						...
						<?php
							...
							foreach ((Array) $data AS $key => $val) { 
								echo '<tr>'.
								'<td>'.$ct.'</td>'.
								'<td><a href="'.site_url('/Payment/detail/'.$val['id']).'" class="blue">'.$val['invoice_number'].'</a></td>'.          
								'<td>'.$val['wo_number'].'</td>'.
								...
								'<td>'.date('Y-m-d H:i:s', strtotime($val['created_dt'])).'</td>'.
								
								'<td>'.($val['dt_lunas'] !== null ? date('Y-m-d H:i:s', strtotime($val['dt_lunas'])) : null).'</td>'.
								'<td>'.$val['no_pintu'].'</td>'.
